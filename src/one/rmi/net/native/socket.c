...
JNI_OnLoad(JavaVM* vm, void* reserved) {
    ...
    f_fd = cache_field(env, "one/rmi/net/Socket", "fd", "I");
...
JNIEXPORT jint JNICALL
Java_one_rmi_net_Socket_socket0(JNIEnv* env, jclass cls) {
...
JNIEXPORT jint JNICALL
Java_one_rmi_net_Socket_accept0(JNIEnv* env, jobject self) {
...
Java_one_rmi_net_Socket_close(JNIEnv* env, jobject self) {
    int fd = (*env)->GetIntField(env, self, f_fd);
    if (fd != -1) {
...
JNIEXPORT void JNICALL
Java_one_rmi_net_Socket_connect(JNIEnv* env, jobject self, jobject address, jint port) {
    int fd = (*env)->GetIntField(env, self, f_fd);
...
JNIEXPORT void JNICALL
Java_one_rmi_net_Socket_bind(JNIEnv* env, jobject self, jobject address, jint port, jint backlog) {
    int fd = (*env)->GetIntField(env, self, f_fd);
    int addr = (*env)->GetIntField(env, address, f_address);
    struct sockaddr_in sa;
    sa.sin_family = AF_INET;
...
JNIEXPORT jint JNICALL
Java_one_rmi_net_Socket_writeRaw(JNIEnv* env, jobject self, jlong buf, jint count) {
    int fd = (*env)->GetIntField(env, self, f_fd);
    if (fd == -1) {
        throw_socket_closed(env);
        return -1;
...
JNIEXPORT void JNICALL
Java_one_rmi_net_Socket_write(JNIEnv* env, jobject self, jbyteArray data, jint offset, jint count) {
    int fd = (*env)->GetIntField(env, self, f_fd);
    ...
    if (fd == -1) {
        ...
        while (count > 0) {
            ...
            if (result <= 0) {
                if (result < 0) throw_exception(env); else throw_socket_closed(env);
                break;
            }
            offset += result;
            count -= result;
...
JNIEXPORT jint JNICALL
Java_one_rmi_net_Socket_readRaw(JNIEnv* env, jobject self, jlong buf, jint count) {
    int fd = (*env)->GetIntField(env, self, f_fd);
    if (fd == -1) {
        throw_socket_closed(env);
        return -1;
...
Java_one_rmi_net_Socket_read(JNIEnv* env, jobject self, jbyteArray data, jint offset, jint count) {
    int fd = (*env)->GetIntField(env, self, f_fd);
    jbyte buf[MAX_STACK_BUF];
...
JNIEXPORT void JNICALL
Java_one_rmi_net_Socket_readFully(JNIEnv* env, jobject self, jbyteArray data, jint offset, jint count) {
    int fd = (*env)->GetIntField(env, self, f_fd);
    jbyte buf[MAX_STACK_BUF];
    ...
    if (fd == -1) {
        ...
        pthread_t* fd_lock = start_blocking_call(fd);
        while (count > 0) {
            int result = recv(fd, buf, count <= MAX_STACK_BUF ? count : MAX_STACK_BUF, MSG_WAITALL);
            if (result <= 0) {
                if (result < 0) throw_exception(env); else throw_socket_closed(env);
                break;
            }
...
JNIEXPORT jint JNICALL
Java_one_rmi_net_Socket_getsockname(JNIEnv* env, jobject self, jbyteArray address) {
    int fd = (*env)->GetIntField(env, self, f_fd);
    struct sockaddr_in sa;
    int len = sizeof(sa);
...
JNIEXPORT jint JNICALL
Java_one_rmi_net_Socket_getpeername(JNIEnv* env, jobject self, jbyteArray address) {
    int fd = (*env)->GetIntField(env, self, f_fd);
    struct sockaddr_in sa;
    int len = sizeof(sa);
...
JNIEXPORT void JNICALL
Java_one_rmi_net_Socket_setTimeout(JNIEnv* env, jobject self, jint timeout) {
    int fd = (*env)->GetIntField(env, self, f_fd);
    struct timeval tv;
...
JNIEXPORT void JNICALL
Java_one_rmi_net_Socket_setKeepAlive(JNIEnv* env, jobject self, jboolean keepAlive) {
    int fd = (*env)->GetIntField(env, self, f_fd);
...
JNIEXPORT void JNICALL
Java_one_rmi_net_Socket_setNoDelay(JNIEnv* env, jobject self, jboolean noDelay) {
    int fd = (*env)->GetIntField(env, self, f_fd);
...
JNIEXPORT void JNICALL
Java_one_rmi_net_Socket_setReuseAddr(JNIEnv* env, jobject self, jboolean reuseAddr) {
    int fd = (*env)->GetIntField(env, self, f_fd);
...
JNIEXPORT void JNICALL
Java_one_rmi_net_Socket_setBufferSize(JNIEnv* env, jobject self, jint recvBuf, jint sendBuf) {
    int fd = (*env)->GetIntField(env, self, f_fd);
